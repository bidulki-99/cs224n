...
class ModelEmbeddings(nn.Module): 
    ...
    def forward(self, input):
        ...
        x_emb = embedding.view(embedding.shape[0] * embedding.shape[1],
                               embedding.shape[3], embedding.shape[2])
        x_conv_out = self.cnn(x_emb)
        x_highway = self.highway(x_conv_out)
        x_word_emb = self.dropout(x_highway).view(
            embedding.shape[0], embedding.shape[1], -1)
